    ...
    BEGIN TRY
		...
		SELECT
			...
			CASE
				...
				ELSE 'n/a'
			END AS gen -- Normalize gender values and handle unknown cases (strip trailing CR/LF from source file)
		...
		SELECT
			...
			END AS cntry -- Normalize and handle missing/blank country codes (strip trailing CR/LF from source file)
		...
		SELECT
			...
			cat,
			subcat,
			TRIM(REPLACE(REPLACE(maintenance, CHAR(13), ''), CHAR(10), '')) AS maintenance -- strip trailing CR/LF from source file
